...
`redirect('/spasibo-za-zakaz.html')` перенаправление на страницу после отправки формы - это не обязательный параметр, если не указан будет возвращен json. (закомментировать для обработки форм с AJAX запросами - это удобно) на счёт этого ниже
